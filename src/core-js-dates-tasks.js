...
function getNextFriday(date) {
  ...
  const receivedWeekDay = receivedDate.getDay();
  const daysUntilFriday = (5 - receivedWeekDay + 7) % 7 || 7;
  const nextFriday = new Date(receivedDate);
  nextFriday.setDate(receivedDate.getDate() + daysUntilFriday);
  return nextFriday;
}
...
function formatDate(date) {
  ...
  const americanTime = receivedDate.toLocaleTimeString('en-Us', options);
  return americanTime;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayOfYear =
    (date - startOfYear + 24 * 3600 * 1000) / (24 * 3600 * 1000);
  let startDay = startOfYear.getDay();
  if (startDay === 0) {
    startDay += 7;
  }
...
function getNextFridayThe13th(date) {
  ...
  throw new Error('Friday, the 13th was not found');
}
